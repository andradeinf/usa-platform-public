	        ...
	        <div class="modal-body">
				<form role="form">
		            <div class="form-group" id="time">
  						...
  						<div class="row">
  							<div class="col-sm-5">
  								<div class="input-group">
		  							...
		  							<input id="inputInitDate" type="text" class="form-control" uib-datepicker-popup="dd/MM/yyyy" ng-model="messagesByTimeRangeReport.initDate" is-open="filter.fromDateOpened" datepicker-options="dateOptions" show-button-bar="False" alt-input-formats="altInputFormats" on-open-focus="False" data-inputmask='"mask": "d/m/y", "showTooltip": "true"' input-mask-initialize />
						...
						<p class="help-block">Período máximo de 3 meses</p>
					</div>
					<div class="form-group" id="franchisee">
						<label for="inputFranchisee"><c:out value="${domainConfiguration.labels['FRANCHISEE']}">Loja</c:out></label>		
						<select id="inputFranchisee" class="form-control" ng-model="messagesByTimeRangeReport.filterFranchiseeId" ng-options="unit.id as unit.name for unit in franchisees | orderBy:'name'"></select>     
					</div> 					
		    	</form>
	        </div>
...
<div class="page-header">
	<h3 class="panel-title"><i class="fa fa-file-text"></i> Relatório de Mensagens dos Franqueados por Período</h3>
</div>
	...
	<div class="panel-body">
		<table class="table table-striped table-bordered history-table">
            <thead>
            	<tr>
					<th>Data</th>
                    <th><c:out value="${domainConfiguration.labels['FRANCHISEE']}">Loja</c:out></th>
                    <th>Período</th>
                    <th>Status</th>
                </tr>
            </thead>
            <tbody>
                <tr ng-repeat="item in messagesByTimeRangeReports | orderBy:'date':true">
					<td>{{item.date | date:'dd/MM/yyyy HH:mm:ss'}}</td>
                    <td>{{franchiseesMap[item.filterFranchiseeId].name}}</td>
